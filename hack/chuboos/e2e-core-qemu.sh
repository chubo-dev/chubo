...
# End-to-end chuboos core flow in QEMU:
...
CLUSTER_NAME="${CLUSTER_NAME:-chuboos-e2e}"
STATE_DIR="${STATE_DIR:-/tmp/chuboos-e2e-state}"
WORKDIR="${WORKDIR:-/tmp/chuboos-e2e-work}"
NODE_IP="${NODE_IP:-10.5.0.2}"
CIDR="${CIDR:-10.5.0.0/24}"
INSTALL_DISK="${INSTALL_DISK:-/dev/vda}"

REGISTRY_NAME="${REGISTRY_NAME:-chuboos-e2e-registry}"
...
SUPPORT_OUT="${SUPPORT_OUT:-/tmp/chuboos-support-e2e.zip}"
CLUSTER_LOGS_OUT="${CLUSTER_LOGS_OUT:-/tmp/logs-chuboos-e2e.tar.gz}"
CLUSTER_SUPPORT_OUT="${CLUSTER_SUPPORT_OUT:-/tmp/support-chuboos-e2e.zip}"
...
echo "building chuboos boot artifacts"
make initramfs kernel sd-boot ARTIFACTS="${ARTIFACTS}" GO_BUILDTAGS="${GO_BUILDTAGS}" PLATFORM="linux/${ARCH}"

echo "building chuboos installer-base and imager docker images"
...
"${TALOSCTL}" gen config chuboos https://0.0.0.0:6443 \
	--with-secrets "${SECRETS_FILE}" \
	-t talosconfig \
	-o "${TALOSCONFIG_FILE}"
...
echo "chuboos core E2E flow completed"
