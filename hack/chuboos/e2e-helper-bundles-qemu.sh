...
# End-to-end smoke for ChuboOS workload helper bundles in QEMU/vmnet:
...
TALOSCTL_CHUBO="${TALOSCTL_CHUBO:-${TALOS_ROOT}/_out/chuboos/talosctl-${HOST_GOOS}-${HOST_GOARCH}}"

REGISTRY_NAME="${REGISTRY_NAME:-chuboos-helper-registry}"
...
RUN_DIR="${RUN_DIR:-$(mktemp -d /tmp/chuboos-helper-e2e.XXXXXX)}"
KEEP_VM="${KEEP_VM:-0}"
...
"${TALOSCTL_BASE}" gen config chuboos https://0.0.0.0:6443 \
	--with-secrets "${SECRETS_FILE}" \
	-t talosconfig \
	-o "${TALOSCONFIG_FILE}"
...
VMNET_ENABLE=1 VMNET_MAC="${VMNET_MAC}" QEMU_RUNDIR="${RUN_DIR}" BOOT_FROM_DISK=0 HOST_PORT="${HOST_PORT}" \
	./hack/qemu/chuboos-qemu.sh >"${LOG_INSTALL}" 2>&1 &
QEMU_INSTALL_PID=$!
...
VMNET_ENABLE=1 VMNET_MAC="${VMNET_MAC}" QEMU_RUNDIR="${RUN_DIR}" BOOT_FROM_DISK=1 HOST_PORT="${HOST_PORT}" \
	./hack/qemu/chuboos-qemu.sh >"${LOG_DISK}" 2>&1 &
QEMU_DISK_PID=$!
