...
if [[ -n "${QEMU_RUNDIR}" ]]; then
  ...
  RUNDIR="$(mktemp -d /tmp/chubo-qemu.XXXXXX)"
...
EOF
  ...
  cat >"${EFIDIR}/loader/entries/chubo.conf" <<EOF
title   ChuboOS (chubo)
