...
# Collect source-clean inventories for the Talos fork and store them
...
cri_pattern='(\bcri\b)'
all_pattern="(${core_pattern}|${cri_pattern})"
...
rg -n -i "${all_pattern}" . >"${audit_dir}/raw-refs.txt" || true
...
rg -n -i "${cri_pattern}" . \
	...
	--glob '!**/*_vtproto.pb.go' \
	--glob '!**/*.binpb' \
	>"${audit_dir}/active-cri-refs.txt" || true
...
if [[ -s "${audit_dir}/chubo-go-buildtag-files.txt" ]]; then
	...
	xargs rg -n -i "${core_pattern}" <"${audit_dir}/chubo-go-buildtag-files.txt" >"${audit_dir}/chubo-go-buildtag-refs.txt" || true
	xargs rg -n -i "${cri_pattern}" <"${audit_dir}/chubo-go-buildtag-files.txt" >"${audit_dir}/chubo-go-buildtag-cri-refs.txt" || true
else
	: >"${audit_dir}/chubo-go-buildtag-refs.txt"
	: >"${audit_dir}/chubo-go-buildtag-cri-refs.txt"
fi
