...
CI_RELEASE_TAG := $(shell git log --oneline --format=%B -n 1 HEAD^2 -- 2>/dev/null | head -n 1 | sed -E -n 's/^release\((.*)\):.*/\1/p')
...
COMMON_ARGS += --build-arg=GO_BUILDFLAGS_CHUBOCTL="$(GO_BUILDFLAGS_CHUBOCTL)"
...
chubo-guardrails: ## Runs chubo-specific regression guardrails (cluster-free image and CLI/API surface).
	...
	@./hack/chubo/check-rootfs.sh _out/chubo/initramfs-$(ARCH).xz
